...
def naiveSoftmaxLossAndGradient(
    ...
    softmaxVec = softmax(outsideVectors.dot(centerWordVec))
    loss = -np.log(softmaxVec[outsideWordIdx])
    gradCenterVec = outsideVectors.T.dot(softmaxVec) - outsideVectors[outsideWordIdx]
    gradOutsideVecs = np.r_[[centerWordVec] * outsideVectors.shape[0]]
    gradOutsideVecs = gradOutsideVecs * softmaxVec.reshape(-1, 1)
...
def negSamplingLossAndGradient(
    ...
    outside_vecs = outsideVectors[np.asarray(indices)]
    sigmoidVec = sigmoid(outside_vecs.dot(centerWordVec))
    loss = -np.log(sigmoidVec[0]) - np.sum(np.log(1-sigmoidVec[1:]))
    gradCenterVec = -outside_vecs[0] * (1 - sigmoidVec[0]) + np.sum(outside_vecs[1:] * sigmoidVec[1:].reshape(-1, 1), axis=0)
    grad_outs = np.r_[[centerWordVec] * len(indices)]
    grad_outs[0] = grad_outs[0] * (sigmoidVec[0] - 1)
    grad_outs[1:] = grad_outs[1:] * sigmoidVec[1:].reshape(-1, 1)
    gradOutsideVecs = np.zeros(np.shape(outsideVectors))
    for i, index in enumerate(indices):
        gradOutsideVecs[index] += grad_outs[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        curLoss, curGrad1, curGrad2 = word2vecLossAndGradient(centerVec, word2Ind[word], outsideVectors, dataset)
        loss += curLoss
        gradCenterVecs[centerIdx] += curGrad1
        gradOutsideVectors += curGrad2
    ### END YOUR CODE
